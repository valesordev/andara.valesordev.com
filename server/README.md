...
| `ingress.idempotency_window` | `ANDARA_INGRESS_IDEMPOTENCY_WINDOW` | `30s` | How long a Submit's outcome is remembered against its `(Session, client_ref)` once known, so a retry inside it is the same Command. Must exceed `ingress.produce_deadline`. Per process; at most `ingress.max_pending` keys per Session, oldest evicted first. |

...
| `snapshot.max_stall_ms` | `ANDARA_SNAPSHOT_MAX_STALL_MS` | `15` | Budget for the in-tick copy — the only part of a round inside the tick, and the part players can feel. A warning, not a refusal: over it, the round continues and `andara_snapshot_failures_total{reason="stall"}` counts it. The sizing fixture's measurement and margin are in `docs/stories/AW-SRV-006-zone-snapshots.md` ("Sizing fixture, measured"). |

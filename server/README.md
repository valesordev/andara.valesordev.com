...
| `snapshot.fs_path` | `ANDARA_SNAPSHOT_FS_PATH` | `/var/lib/andara/snapshots` | Directory the `fs` store writes under, keyed `{zone_id}/{state_version}/{offset}` with the offset zero-padded to 20 digits so a lexical listing is offset-ordered. Writes go to `{key}.tmp` and are renamed, so an interrupted write leaves nothing a listing returns. |

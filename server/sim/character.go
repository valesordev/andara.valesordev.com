...
func applyBindCharacter(a *ApplyContext, cmd *logv1.LoggedCommand) error {
	...
	if ent, ok := a.Zone.Entities[id]; ok {
		...
		a.bound(bind, id, a.Zone.ID, ent.Room, BindWoken)
		return nil
	}
...
func (a *ApplyContext) bound(bind *logv1.BindCharacter, id EntityID, zone ZoneID, room RoomID, body BindBody) {
	a.bind = &BindResult{Account: bind.GetAccountId(), Character: id, Zone: zone, Room: room, Body: body}
}
